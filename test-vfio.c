...
int main(int argc, char * argv[]){
  ...
  if(argc>1 ){
    ...
    if(mem_type > 2){ PERR("invalid memory type (1: dummymap, 2: devdax map)"); return -1;}
  }

  PINF("Using memory type %d(0: anonymous mmap, 1: dummymap, 2: devdax map)", mem_type);
